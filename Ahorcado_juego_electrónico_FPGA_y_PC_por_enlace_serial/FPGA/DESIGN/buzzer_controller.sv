...
//
// Genera una onda cuadrada distinta segun lo que acabe de pasar en el
// juego:
//
...
module buzzer_controller #(
    ...
    // Una onda cuadrada invierte su salida cada medio periodo, asi que
    // lo que hace falta saber es cuantos ciclos de reloj dura esa mitad.
    // Se suma f antes de dividir para redondear al entero mas cercano en
    // vez de truncar siempre hacia abajo.
    ...
    // El tono mas grave da el semiperiodo mas largo, asi que es el que
    // decide cuantos bits necesita el contador.
    ...
    // Segun el evento, y en las secuencias segun el tono en que se va,
    // se escogen el semiperiodo y la duracion que corresponden.
    ...
    // Un sonido nuevo se acepta solo si no hay ninguno sonando. La
    // excepcion son los de fin de partida, que cortan lo que este
    // sonando porque son los que no se pueden perder.
    logic es_fin, acepta;
    ...
    always_ff @(posedge clk_i) begin
        if (rst_i) begin
            ...
            // la salida, y de ahi sale la onda cuadrada.
            ...
            // al final se pasa al siguiente tono, o se apaga todo si ese
            // era el ultimo.
            if (tick_i) begin
